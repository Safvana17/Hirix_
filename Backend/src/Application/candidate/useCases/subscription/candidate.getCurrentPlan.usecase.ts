...
import { ICandidateGetCurrentPlanUsecase } from "../../interfaces/subscription/ICandidate.getCurrentPlan.usecase";

export class CandidateGetCurrentPlanUsecse implements ICandidateGetCurrentPlanUsecase {
    constructor(
    ...
    async execute(request: CandidateGetCurrentPlanInputDTO): Promise<CandidateGetCurrentPlanOutputDTO> {
        const candidate = await this._candidateRepository.findById(request.id)

        if(!candidate){
            throw new AppError(authMessages.error.COMPANY_NOT_FOUND, statusCode.NOT_FOUND)
        }
        let subscription = await this._subscriptionRepository.findCurrentByUserId(candidate.id)

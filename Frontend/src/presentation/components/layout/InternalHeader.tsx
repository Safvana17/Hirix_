...
//     const { notifications } = useSelector((state: RootState) => state.AdminSettings)
  ...
  return (
    <header className="w-full h-16 bg-[#f5f0e8] border-b border-white flex items-center justify-between px-8 sticky top-0 z-50">
      <div className="flex items-center gap-2 mr-5">
        ...
        <span className="font-irish text-Black text-4xl font-bold tracking-[0.15em]">
          HiriX
        </span>
      </div>
      ...
      <div className="flex items-center gap-6 right-0">
        <div className="relative" ref={notificationRef}>
          <button
            ...
            className="p-2.5 text-[#7c5a1a]/70 hover:text-[#7c5a1a] hover:bg-[#e9e2d5] rounded-xl transition-all relative"
          >
            <Bell className="w-6 h-6" />

            {hasUnreadNotifications && (
              <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#f5f0e8]" />
            )}
          </button>

          {isNotificationOpen && (
            <div className="absolute right-0 mt-3 w-[360px] bg-white rounded-2xl shadow-xl border border-[#e2d8c3] z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                ...
                {hasUnreadNotifications && (
                  <button
                    ...
                    className="text-xs font-medium text-[#7c5a1a] hover:text-[#5f4314] transition-colors"
                  >
                    Mark all read
                  </button>
                )}
              ...
              <div className="max-h-[420px] overflow-y-auto">
                {!notifications || notifications.length === 0 ? (
                  ...
                  notifications.map((notification) => (
                    <div
                      key={notification.id}
                      className={`px-4 py-3 border-b border-gray-100 hover:bg-[#faf7f1] transition-colors ${
                        !notification.isRead ? 'bg-[#fffaf2]' : 'bg-white'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-xs text-gray-600 leading-5 break-words">
        ...
        <div className="relative" ref={dropDownRef}>
          ...
          >
            ...
            <ChevronDown
              className={`w-4 h-4 text-[#7c5a1a] transition-transform duration-200 ${
